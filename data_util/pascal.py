...
class DatasetPASCAL(Dataset):
    def __init__(self, datapath, transform, split, shot):
        ...
        self.img_path = os.path.join('../data/VOCdevkit/VOC2012/JPEGImages/')            # Picture path
        self.ann_path = os.path.join('../data/VOCdevkit/VOC2012/SegmentationClassAug/')  # Mask path
        self.transform = transform

        self.class_ids = self.build_class_ids()  # Build a list of category ids
        self.img_metadata = self.build_img_metadata()  # Building an image metadata list
        self.img_metadata_classwise = self.build_img_metadata_classwise()  # Constructing a dictionary of image metadata grouped by category
    ...
    #  Convert the mask into a binary grayscale image
    ...
    # Read the mask of the image and return it as a PIL image object
    def read_mask(self, img_name):
        r"""Return segmentation mask in PIL Image"""
        mask = torch.tensor(np.array(Image.open(os.path.join(self.ann_path, img_name) + '.png')))
        return mask

    # Reads an image and returns it as a PIL image
    ...
    def build_img_metadata(self):
        ...
        img_metadata = []
        if self.split == 'trn':  # In the training stage, the metadata list of all folds is read
        ...
        print('Total (%s) images are from VOC: %d' % (self.split, len(img_metadata)))

        return img_metadata

    # Constructing a dictionary of image metadata classified by category
