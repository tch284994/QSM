...
class DatasetISIC(Dataset):
    def __init__(self, datapath, transform, split, shot, num=600):
        self.split = split
        self.benchmark = 'isic'
        self.shot = shot
        self.num = num

        self.base_path = os.path.join(datapath, 'ISIC')  # Basic path of dataset
        self.categories = ['1','2','3']  # Number of categories
    ...
    def __getitem__(self, idx):  # Get a sample of a specific index idx in the dataset
    ...
    # Read the mask of the image and convert it into a grayscale image
    ...
    def sample_episode(self, idx):
        ...
        while True:
            ...
            if len(support_names) == self.shot: break  # Stop sampling when n-shot is reached

        return query_name, support_names, class_id

    # Constructing image metadata
    def build_img_metadata(self):
        ...
        print('Total (%s) images are from ISIC: %d ' % (self.split, len(img_metadata)))

        return img_metadata

    # Used to sample image metadata by category
    def build_img_metadata_classwise(self):
        ...
        for cat in self.categories:
            ...
            total_images += len(img_paths)  # Total number of pictures to be processed
            for img_path in img_paths:
                if os.path.basename(img_path).split('.')[1] == 'jpg':
                    img_metadata_classwise[cat] += [img_path]

        print('Total (%s) images are from isic: %d ' % (self.split, self.num))
