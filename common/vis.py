r""" Visualization of prediction results """
import os
...
class Visualizer:  # Mask Visualizer

    @classmethod
    def initialize(cls, vis_path, visualize):  # Visualize, which is used to indicate whether to visualize
        cls.visualize = visualize
        if not visualize:  # visualize or not
            return

        # If visualize is True, some initialization operations are performed
        ...
        cls.to_pil = transforms.ToPILImage()  # Transform tensor into PIL image
        cls.vis_path = vis_path  # Save path of visualization results
        if not os.path.exists(cls.vis_path): os.makedirs(cls.vis_path)  # If the path does not exist, create it

    @classmethod  # Visualize the forecast results of each batch
    def visualize_prediction_batch(cls, spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b, batch_idx, iou_b=None):
        spt_img_b = utils.to_cpu(spt_img_b)  # Transfer to CPU for processing
    ...
    @classmethod  # Converts a tensor to a NumPy array
    ...
    def visualize_prediction(cls, spt_imgs, spt_masks, qry_img, qry_mask, pred_mask, cls_id, batch_idx, sample_idx, label, iou=None):
        ...
        spt_color = cls.colors['blue']  # Different samples are marked with different colors
        ...
        # Merge into one PIL image object
        merged_pil = cls.merge_image_pair(spt_masked_pils + [qry_masked_pil, pred_masked_pil])
        iou = iou.item() if iou else 0.0

        # Save the merged image in JPEG format with the file name 'batch_idx_sample_idx_class-cls_id_iou-iou.jpg'
        merged_pil.save(cls.vis_path + '%d_%d_class-%d_iou-%.2f' % (batch_idx, sample_idx, cls_id, iou) + '.jpg')

    @classmethod
    def merge_image_pair(cls, pil_imgs):  # Align a set of PIL image objects horizontally
    ...
    @classmethod
    def apply_mask(cls, image, mask, color, alpha=0.5):  # Apply the given mask to the image
